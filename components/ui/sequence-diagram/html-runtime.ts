...
.mark{position:absolute;right:12px;bottom:12px;font:11px ui-monospace,monospace;color:var(--muted);letter-spacing:.04em}
...
.ll{position:absolute;width:0;border-left:1px dashed var(--line);z-index:1;transition:border-color 120ms}
.ll.active{border-left-color:var(--accent);border-left-style:solid}
.actor{position:absolute;width:132px;height:52px;display:flex;align-items:center;justify-content:center;gap:8px;padding:0 12px;text-align:center;border:1px solid var(--line);background:var(--card);font:600 13px ui-sans-serif,system-ui,sans-serif;z-index:5;transition:border-color 120ms,box-shadow 120ms}
.actor.active{border-color:var(--accent);box-shadow:0 0 0 1px var(--accent)}
...
.msg-line{stroke:var(--accent);stroke-width:1.5;fill:none;transition:opacity 120ms,stroke-width 120ms}
.msg-line.dim{opacity:.35}
.msg-line.hl{stroke-width:2.6}
.hit{stroke:transparent;stroke-width:22;fill:none;pointer-events:stroke;cursor:default}
.dot-fly{position:absolute;left:0;top:0;width:9px;height:9px;margin:-4.5px 0 0 -4.5px;border-radius:50%;background:var(--accent);opacity:0;z-index:7;pointer-events:none}
.dot-fly.run{animation:fly var(--travel,650ms) cubic-bezier(.4,0,.2,1) forwards;animation-delay:var(--delay,0ms)}
.msg-label{position:absolute;transform:translate(-50%,-50%);z-index:8;background:var(--accent);color:var(--accent-fg);padding:3px 8px;font:600 11px ui-sans-serif,system-ui,sans-serif;white-space:nowrap;cursor:default;transition:opacity 120ms,box-shadow 120ms}
.msg-label.dim{opacity:.4}
.msg-label.hl{box-shadow:0 0 0 2px color-mix(in oklab,var(--accent) 40%,transparent)}
.tip{position:absolute;transform:translate(-50%,calc(-100% - 10px));z-index:20;max-width:240px;padding:7px 10px;background:var(--fg);color:var(--bg);font:12px/1.4 ui-sans-serif,system-ui,sans-serif;border-radius:4px;pointer-events:none;opacity:0;transition:opacity 120ms;text-align:center}
.tip.show{opacity:1}
...
.made{margin:18px 2px 0;text-align:center;font:12px ui-monospace,monospace;color:var(--muted);letter-spacing:.04em}
.sr{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0,0,0,0)}
@keyframes rise{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
@keyframes fadein{to{opacity:1}}
@keyframes fly{0%{offset-distance:0%;opacity:0}12%{opacity:1}88%{opacity:1}100%{offset-distance:100%;opacity:0}}
.anim-rise{opacity:0;animation:rise .45s ease forwards}
.anim-fade{opacity:0;animation:fadein .28s ease forwards}
@media (max-width:760px){.stage-wrap{height:440px}}
@media (prefers-reduced-motion:reduce){.anim-rise,.anim-fade,.dot-fly.run{animation:none;opacity:1}.dot-fly.run{opacity:0}}`
...
const STAGGER=70,TRAVEL=650,MS_PER_WORD=60000/160,ORIENT=600,MIN_HOLD=1600,MAX_HOLD=5000,SETTLE_PAD=150;
...
function holdMs(t){const w=(String(t||'').trim().match(/\\S+/g)||[]).length;return Math.min(MAX_HOLD,Math.max(MIN_HOLD,w*MS_PER_WORD+ORIENT));}
function extractTip(raw){
const k=raw.indexOf('%%');if(k<0)return {line:raw.trim()};
const before=raw.slice(0,k).trim(),body=raw.slice(k+2).trim();
const pm=body.match(/^tooltip\\s+([^:]+):\\s*(.+)$/i);
if(pm)return {line:before,ptip:{name:pm[1].trim().toLowerCase(),text:pm[2].trim()}};
let exp,dur;body.split('|').forEach(part=>{const m=part.trim().match(/^(tooltip|duration)\\s*:\\s*(.+)$/i);if(m){if(m[1].toLowerCase()==='tooltip')exp=m[2].trim();else{const ms=parseInt(m[2],10);if(ms>0)dur=ms;}}});
return {line:before,explanation:exp,durationMs:dur};
}
function parse(input){
const participants=[],byId=new Map(),boxes=[],events=[],labelNotes=new Map();let box=null,idx=0;
...
input.split(/\\r?\\n/).forEach(raw=>{
const tip=extractTip(raw);if(tip.ptip){labelNotes.set(tip.ptip.name,tip.ptip.text);if(!tip.line)return;}
const line=tip.line;if(!line)return;const low=line.toLowerCase();if(low==='sequencediagram')return;
...
if(m){const pl=m[1].toLowerCase().charAt(0)==='r'?'right':m[1].toLowerCase().charAt(0)==='l'?'left':'over';const ids=m[2].split(',').map(s=>s.trim()).filter(Boolean);ids.forEach(id=>ensure(id));events.push({kind:'note',index:idx++,placement:pl,ids:ids,text:m[3].trim(),explanation:tip.explanation,durationMs:tip.durationMs});return;}
...
if(found){const from=line.slice(0,found.at).trim();let after=line.slice(found.at+found.token.length),text='';const c=after.indexOf(':');if(c>=0){text=after.slice(c+1).trim();after=after.slice(0,c);}let target=after.trim(),actT=false,deS=false;if(target.charAt(0)==='+'){actT=true;target=target.slice(1).trim();}else if(target.charAt(0)==='-'){deS=true;target=target.slice(1).trim();}if(from&&target){ensure(from);ensure(target);events.push({kind:'message',index:idx++,from:from,to:target,text:text,line:found.line,head:found.head,self:from===target,actT:actT,deS:deS,explanation:tip.explanation,durationMs:tip.durationMs});}}
});
participants.forEach(p=>{const t=labelNotes.get(p.id.toLowerCase())||labelNotes.get(p.label.toLowerCase());if(t)p.explanation=t;});
boxes.forEach(b=>{const t=labelNotes.get(b.label.toLowerCase());if(t)b.explanation=t;});
...
const settle=model.participants.length*STAGGER+SETTLE_PAD;let cursor=settle;
...
if(ev.kind==='message'){const my=y,y2=ev.self?my+SELF_H:my;messages.push(Object.assign({},ev,{y1:my,y2:y2,enterDelay:cursor}));if(ev.actT)push(ev.to,my);if(ev.deS)pop(ev.from,y2);y=(ev.self?y2:my)+MSG_GAP;cursor+=(ev.durationMs||holdMs(ev.text));}
else if(ev.kind==='note'){const xs=ev.ids.map(xOf);let nx,nw;if(ev.placement==='over'){const mn=Math.min.apply(null,xs),mx=Math.max.apply(null,xs);nw=ev.ids.length>1?mx-mn+ACTOR_W:150;nx=(mn+mx)/2-nw/2;}else if(ev.placement==='right'){nx=xOf(ev.ids[0])+16;nw=150;}else{nw=150;nx=xOf(ev.ids[0])-16-nw;}notes.push(Object.assign({},ev,{x:nx,y:y,width:nw,enterDelay:cursor}));y+=NOTE_H+20;cursor+=(ev.durationMs||holdMs(ev.text));}
...
const canvas=$('canvas'),stage=$('stage'),tip=$('tip');
...
canvas.innerHTML='';tip.classList.remove('show');
...
const delay=id=>reduce?0:(order.get(id)||0)*STAGGER;
...
const llEls={},actorEls={};
model.participants.forEach(p=>{
const x=graph.xOf(p.id);
const ll=el('div','ll',{left:x+'px',top:graph.llTop+'px',height:graph.llHeight+'px'});llEls[p.id]=ll;canvas.appendChild(ll);
actorEls[p.id]=[];
[graph.headY,graph.bottomHeadY].forEach(yy=>{const a=el('div','actor anim-rise',{left:(x-ACTOR_W/2)+'px',top:yy+'px',animationDelay:delay(p.id)+'ms'},(p.actor?'\\u25EF  ':'')+p.label);actorEls[p.id].push(a);canvas.appendChild(a);});
});
...
graph.notes.forEach(n=>{canvas.appendChild(el('div','note anim-rise',{left:n.x+'px',top:n.y+'px',width:n.width+'px',animationDelay:(reduce?0:n.enterDelay)+'ms'},n.text));});
...
lineEls[k].classList.toggle('hl',k===n);lineEls[k].classList.toggle('dim',n!==null&&k!==n);
if(labelEls[k]){labelEls[k].classList.toggle('hl',k===n);labelEls[k].classList.toggle('dim',n!==null&&k!==n);}
});
const act=n===null?null:{a:graph.messages[n].from,b:graph.messages[n].to};
Object.keys(llEls).forEach(pid=>{const on=!!act&&(pid===act.a||pid===act.b);llEls[pid].classList.toggle('active',on);actorEls[pid].forEach(e=>e.classList.toggle('active',on));});
if(n!==null&&graph.messages[n].explanation){const m=graph.messages[n],lx=m.self?graph.xOf(m.from)+62:(graph.xOf(m.from)+graph.xOf(m.to))/2,ly=m.self?(m.y1+m.y2)/2:m.y1;tip.textContent=m.explanation;tip.style.left=(lx*view.scale+view.tx)+'px';tip.style.top=(ly*view.scale+view.ty)+'px';tip.classList.add('show');}else{tip.classList.remove('show');}
}
...
if(!reduce)path.style.animationDelay=m.enterDelay+'ms';
svg.appendChild(path);lineEls[n]=path;
const hit=document.createElementNS(NS,'path');hit.setAttribute('d',d);hit.setAttribute('class','hit');hit.addEventListener('mouseenter',()=>setHover(n));hit.addEventListener('mouseleave',()=>setHover(null));svg.appendChild(hit);
if(!reduce){const fly=el('div','dot-fly',{offsetPath:'path("'+d+'")'});fly.style.setProperty('--travel',TRAVEL+'ms');fly.style.setProperty('--delay',m.enterDelay+'ms');canvas.appendChild(fly);requestAnimationFrame(()=>fly.classList.add('run'));}
if(m.text){const lx=m.self?x1+62:(x1+x2)/2,ly=m.self?(m.y1+m.y2)/2:m.y1;const lab=el('div','msg-label anim-fade',{left:lx+'px',top:ly+'px',animationDelay:(reduce?0:m.enterDelay+TRAVEL)+'ms'},m.text);lab.addEventListener('mouseenter',()=>setHover(n));lab.addEventListener('mouseleave',()=>setHover(null));canvas.appendChild(lab);labelEls[n]=lab;}
});
fit();
}
function renderStep(){
...
function setPlaying(v){playing=v;$('play').textContent=v?'Pause':'Play';if(timer){clearInterval(timer);timer=null;}if(v)timer=setInterval(()=>{i=(i+1)%slides.length;renderStep();},5200);}
...
${STYLE}</style></head>
<body><main class="shell"><div class="top"><span class="eyebrow">Sequence Flow / exported journey</span><button class="action" id="theme">Theme</button></div><section class="frame" aria-label="Journey player"><div class="copy"><span class="eyebrow" id="count"></span><h1 id="title"></h1><p class="caption" id="caption"></p></div><div class="stage-wrap"><div class="stage" id="stage"><div class="canvas" id="canvas"></div><div class="tip" id="tip"></div><div class="zoom" aria-label="Canvas controls"><button id="zoomIn" title="Zoom in">+</button><button id="zoomOut" title="Zoom out">&minus;</button><button id="fit" title="Fit view">Fit</button></div><span class="mark">Sequence Flow</span></div></div><div class="controls"><div class="steps" id="steps"></div><div class="btns"><button class="action" id="prev">Prev</button><button class="action" id="play">Play</button><button class="action" id="next">Next</button></div></div></section><p class="made">Built in Montréal by Canadian AI</p><p class="sr" aria-live="polite" id="live"></p></main>
